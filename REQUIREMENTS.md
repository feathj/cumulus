...
* Knowledge entries
  ...
  * Each entry holds exactly one thing, so it can be updated, replaced, or retired on its own:
  ...
  * Entries can link to each other with [[wiki links]]
...
* Events
  * An append-only timeline of what happened
    ...
    * Things that happened in the world: a meeting, a release, a test run
  * Events record which knowledge entries they created, changed, or superseded
  * Events record where they came from: which LLM, its session, and a related commit or link
...
* Memory attachments
  * Images and documents can be attached to knowledge entries and events
  * Attachments carry a text description so an LLM can use them without loading the file
